    ...
    public class Job
    {
        ...
        private void Transcode()
        {
            ...
            try
            {
                ...
                // Log all video/audio streams and collect non-MJPEG ones
                var VideoStreams = new List<IVideoStream>();
                var AudioStreams = new List<IAudioStream>();

                foreach (var stream in tasks[0].Result.VideoStreams)
                {
                    System.Diagnostics.Debug.WriteLine($"Stream {stream.Index}: Codec={stream.Codec}");

                    if (stream.Codec != "mjpeg")
                    {
                        VideoStreams.Add(stream); // Add non-MJPEG streams
                    }
                }

                foreach (var stream in tasks[0].Result.AudioStreams)
                {
                    //System.Diagnostics.Debug.WriteLine($"Stream {stream.Index}: Codec={stream.Codec}");
                ...
                {
                    Task<IConversion> converter;
                    converter = FFmpeg.Conversions.FromSnippet.Concatenate(outputFile, inputVideos);
                    converter.Wait(ct);
                ...
                else
                {
                    ...
                    conversion.AddStream(VideoStreams);
                    conversion.AddStream(AudioStreams);
                }
                ...
                //System.Diagnostics.Debug.WriteLine("Ffmpeg Command generated by Xabe: " + conversion.Build()); // Writes the full ffmpeg command to console for debugging
